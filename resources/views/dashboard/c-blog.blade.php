...
<form action="/dashboard/blog/create" method="POST" enctype="multipart/form-data" class="space-y-4">

    <div class="grid">
        @csrf

        <label class="font-semibold">Blog Title</label>
        <input name="title" type="text" placeholder="Type Title Here" class="font-semibold" required>
    </div>
    <label for="choose-file" class="flex flex-col gap-3 hover:cursor-pointer">
        <p>Blog Thumbnail</p>
        <div id="img-preview" class="border-2 border-neutral-400 border-dashed rounded-md h-96 p-3 flex flex-col justify-center items-center">
...
function getImgData(inputId, previewId) {
    ...
    if (files) {
        ...
        fileReader.addEventListener("load", function () {
            imgPreview.innerHTML = '<img class="max-h-full max-w-full" src="' + this.result + '" />';
        });
    }
}
</script>
